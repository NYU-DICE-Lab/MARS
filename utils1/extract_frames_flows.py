...
#             # extract flows 
#             os.system('export CUDA_VISIBLE_DEVICES=%d; %s/utils1/tvl1_videoframes "%s" %d "%s" '%(gpu, os.getcwd(), os.path.join(outdir, '%05d.jpg'), nframes, os.path.join(outdir,'%05d_flow256crop.flo') ))
...
def saveFlow(videoItem):
    gpuId,cls,frame_dir,redo,v = videoItem
    outdir = os.path.join(frame_dir, cls, v[:-4])
    if not os.path.isfile(os.path.join(outdir, 'done')) or redo:
        print("*" * 50)
        print(gpuId, cls, frame_dir, redo, v, outdir)
        print("*" * 50)
        try:
            os.system('mkdir -p "%s"' % (outdir))
            # check if horizontal or vertical scaling factor
...
def extract_flows(vid_dir, frame_dir, start, end, gpu, redo=False):
    ...
    print("Classes =", class_list)
    for ic, cls in enumerate(class_list):
        # change to caculate in multithread
        thread_num = 25
        ...
        print(ic + 1, len(class_list), cls, len(vlist))
        print("")
        # gpuId, cls, frame_dir, redo, v
        videoItems = [(gpu,cls,frame_dir,redo, v) for v in vlist]
        p.map(saveFlow, videoItems)
        p.close()
        p.join()
        print(ic,"class :",cls, "end")
